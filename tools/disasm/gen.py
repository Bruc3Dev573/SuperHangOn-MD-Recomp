...
class Generator:
    ...
    def run(self, out):
        ...
        w("\torg\t0\n\n")
        # RAM block equates
        for name, value in getattr(self, "defines", []):
            w("%s\tequ\t%s\n" % (name, value))
...
def main():
    ...
    ap.add_argument("--define", action="append", default=[],
                    help="NAME=VALUE equate at the top of the source (for overlays)")
    ...
    with open(args.out, "w") as f:
        g = Generator(rom, an, len(rom), load_pointers(args.pointers), shifts,
                      load_overlays(args.overlay))
        g.defines = [tuple(d.split("=", 1)) for d in args.define]
        g.run(f)
