...
usage: recomp.py IMAGE LISTING OUT.c [--symbols symbols.txt] [--code-map MAP.c --code-set NAME]
...
addresses, as the code set rt_code_set_NAME (with the overlays' bytes
rom_patches_NAME of tools/recomp/romdiff.py). The runtime translator
(port/src/rt_translate.c) decodes the instructions from the user's ROM with it
and runs them as these blocks.
"""
import argparse
import os
import re
...
def main():
    ...
    ap.add_argument("--code-set", default="default")
    a = ap.parse_args()
    ...
    if a.code_map:
        ...
        m.append("static const uint32_t waits[] = {")
        m.extend("  0x%06x," % w for w in wl)
        if not wl:
            m.append("  0,")
        m.append("};")
        m.append("")
        m.append("extern const RomPatch rom_patches_%s[];" % name)
        m.append("extern const int rom_patch_count_%s;" % name)
        m.append("const RtCodeSet rt_code_set_%s = {blocks, %d, waits, %d, rom_patches_%s, &rom_patch_count_%s};" % (
            name, len(code_map), len(wl), name, name))
